...
public class GScoreController extends ScoreSheetsController{
    ...
    public void updateGreenScoreSheet(){
        GreenRealm greenRealm=player.getGreenRealm();
        boolean[] gaiaHealth=greenRealm.getGaia().getGurdiansHealth();
        
        Label[] values={S2,S3,S4,S5,S6,S7,S8,S9,S10,S11,S12};
        for(int i=2;i<gaiaHealth.length;i++)
            if(gaiaHealth[i])
                values[i-2].setText(""+i);
            else
                values[i-2].setText("X");    

        String[] drawRew=greenRealm.getDrawRewards();        
        ImageView[] rewards={R1,R2,R3,R4,R5,R6,R7};
    ...
    public void initialize() {
        ...
        theScoreSheet.setImage(new Image(getClass().getResourceAsStream("/images/greenScoreSheet.png")));
