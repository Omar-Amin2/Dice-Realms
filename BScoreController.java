...
public class BScoreController extends ScoreSheetsController{
    ...
    public void updateBlueScoreSheet(){
        BlueRealm blueRealm=player.getBlueRealm();
        String[] prevAt=blueRealm.getPreviousAttacks();        
        Label[] values={S1,S2,S3,S4,S5,S6,S7,S8,S9,S10,S11};
        for(int i=0;i<values.length;i++)
            if(prevAt[i].charAt(0)=='-')
                values[i].setText("--");
            else    
                values[i].setText(prevAt[i]);
             

        String[] drawRew=blueRealm.getDrawRewards();        
    ...
    public void initialize() {
        ...
        theScoreSheet.setImage(new Image(getClass().getResourceAsStream("/images/blueScoreSheet.png")));
